...
def play_poker_hand(hand1, hand2):
  """
  Play two player against each other
  """
  ...
  #Print the hands as well as the rand and type of hand
  print(f"Hand 1: {hand1}-{hand1_rank}-{hand1_rank_value}")
  print(f"Hand 1: {hand2}-{hand2_rank}-{hand2_rank_value}")
...
@cli.command("info")
@click.option("--probability", is_flag=True, help="Display the probobility of each hand of poker")
def info(probability):
  """
  Display all possible winning hands with examples
  """
  ...
  if probability:
    ...
    for hand, _ in poker_hand_rank.items():
      #Print probability of each hand with click colour and example and 1 in x chance
      click.secho(
        "{} - {} - 1 in {:,} - {:4f}%".format(
          hand,
          poker_hand,
...
@click.option("name", "--name", default="Player", help="Name of the player")
@click.option("rounds", "--rounds", default=1, help="Numbe of rounds")
@click.option("money", "--money", default=1000, help="Amount of money start with")
def interactive(name, rounds, money):
  ...
  for i in range(1, rounds + 1):
    ...
    #As user how much they want to be bet
    bet = click.prompt("How Much wouldy you like to bet?", type=int)
    status = play_poker_hand(hand1, hand2)
